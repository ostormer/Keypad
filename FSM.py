...
class FiniteStateMachine:
    ...
    def __init__(self):
        ...
        self.add_rule("s-init", "s-read1", all_symbols, "a1")  # tar oss fra init til read via @
        ...
        self.add_rule("s-read1", "s-init", all_symbols, "a14")  # sender oss tilbake til init ved "#"

        self.add_rule("s-verify1", "s-active", "Y", "a4")  # sender oss til s-active om passord er rett
        self.add_rule("s-verify1", "s-read1", all_symbols, "a4")  # sender oss til s-init om passord er feil
        ...
        self.add_rule("ANY_STATE", "s-logout", "#", "a4")  # etter s-active vil "#" alltid sende oss til logout

        self.add_rule("s-active", "s-led", led_id, "a7")  # sender oss til led id ved {0,5]
        self.add_rule("s-active", "s-read2", "*", "a4")  # samme passord prosess som ved oppstart
        self.add_rule("s-active", "s-active", all_symbols, "a4")  # ved tall høyere enn 5, gjør ingenting
        ...
        self.add_rule("s-verify2", "s-read3", "Y", "a4")  # passord riktig, sender oss til "skriv nytt passord"
        self.add_rule("s-verify2", "s-active", all_symbols, "a4")  # passord feil, sender oss til s-active

        self.add_rule("s-read3", "s-read3", all_digits, "a2")  # append digit i nytt passord
        self.add_rule("s-read3", "s-active", "*", "a12")  # valider at nytt passord har 4 chars og sett som nytt passord
    ...
    def run_rules(self):
        print("Looking for rules: Current state: %s,  Current symbol: %s" % (self.current_state, self.current_symbol))
